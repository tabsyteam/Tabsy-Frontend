...
import {
  Filter, RefreshCw, AlertCircle, Plus, Search, Grid, List,
  Package, Utensils, TrendingUp, BarChart3, Eye, Sparkles,
  SlidersHorizontal
} from 'lucide-react'
...
export function MenuManagement({ restaurantId }: MenuManagementProps) {
  ...
  const handleEdit = (data: MenuCategory | MenuItem) => {
    ...
    if ('categoryId' in data || 'status' in data || 'basePrice' in data || 'preparationTime' in data) {
      // It's a menu item
  ...
  const averageItemsPerCategory = totalCategories > 0 ? (totalItems / totalCategories).toFixed(1) : '0'
      ...
      {!categoriesLoading && !itemsLoading && !(categoriesError || itemsError) && (
        <>
          {getFilteredData().length === 0 ? (
            <EmptyState
              type={searchQuery ? 'no-results' : viewFilter === 'categories' ? 'categories' : viewFilter === 'items' ? 'items' : 'no-data'}
              title={
                searchQuery
                  ? 'No matching results'
